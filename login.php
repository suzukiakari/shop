...
if(!empty($_POST)) {
  ...
  if($id != '' && $password != '') {
      ...
      $login = $db->prepare('SELECT id, mail, password FROM member WHERE mail = ? AND password = ?');
      $login->execute(array(
          $id,
          $_POST['password']
      ));
      ...
      if($resultAdmin) {
        ...
        $_SESSION['member_id'] = $resultAdmin['id'];
        $_SESSION['time'] = time();
...
//入力内容が空であればメッセージを表示
if($error == 'failed') {
  print '<div class="error">＊ユーザーIDもしくはパスワードが違います</div>';
}
if($error == 'blank') {
